...
footerLinks.forEach((link) => link.addEventListener("click", displayClickedItems))
...
function fillTodoList(action) {
    ...
    if (todos.length == 0) {
        ...
        todos.forEach((item) => {
            ...
            if (item.isCompleted) {
                ...
                circle_wrap.style.backgroundImage = getComputedStyle(document.documentElement).getPropertyValue(
                    "--color-blue-grad"
                )
            ...
            if (uncompleted > 0) {
                document.querySelector(".todo_count").innerHTML = uncompleted + " items left"
            }
    ...
    checkInputs.forEach((item) => item.addEventListener("click", handleCompleted))
}
